...
const splitDate=(date1)=>{
    ...
    const formattedDate = isoDate.substring(0, isoDate.indexOf('T'));
    return formattedDate
}
export default function EditBooking(){
    const location=useLocation()
    const navigate=useNavigate()
  console.log("splitdate",(splitDate(location.state.showtime_id.showtime_date_time)))
    const [selectedMovie, setSelectedMovie] = useState("")
    const [showtimes, setShowtimes] = useState([])
    const [selectedDate, setSelectedDate] = useState(splitDate(location.state.showtime_id.showtime_date_time) ||'')
    const [selectedTimeId, setSelectedTimeId] = useState(location.state.showtime_id._id || '')
    ...
    useEffect(() => {
        const movieId=location.state.showtime_id.movie_id._id        
      if (selectedDate) {
        (async () => {
          try {
            const response = await axios.get(`http://localhost:3001/api/showtime/${movieId}?date=${selectedDate}`)
    ...
    const handleBookSeats = async () => {
      ...
      try {
        const response = await axios.put(`http://localhost:3001/api/booking/update/${location.state._id}`, formData, {
          headers: {
            Authorization: localStorage.getItem("token")
          }
        })
        // console.log("booking response",response.data)
        toast.success(" booking Edit  succesfull")
        navigate("/mybookings")
      } catch (err) {
        console.log(err)
      }
